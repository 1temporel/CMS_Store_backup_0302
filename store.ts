...
export interface GhostItemData {
    _id: string
    _index: number
    [key: string]: any
}
...
export interface FilterState {
    ...
    items: GhostItemData[] // Stockage des métadonnées des items (données pures, pas de DOM)
    filteredItems: GhostItemData[]
    pagination: {
        currentPage: number
        itemsPerPage: number
        totalItems: number
 ...
 * Utilise le pattern Observer pour notifier les composants (ActiveTags, DisplayGrid, etc.).
 *
 * GHOST PATTERN: Le store ne gère que des données pures (JSON).
 * Les éléments DOM ne sont plus référencés dans le store.
 */
class FilterStore {
    ...
    private itemsMap: Map<string, GhostItemData> = new Map() // Pour dédoublonnage rapide
    ...
    // --- GHOST PATTERN METHODS ---

    /**
     * Enregistre un item "fantôme" dans le store.
     * Utilisé par GhostItem.tsx pour alimenter le store avec les données CMS.
     * Dédoublonnage automatique basé sur l'ID.
     */
    registerGhostItem(item: GhostItemData): void {
        this.lazyInit()

        if (!item._id) {
            console.warn("GhostItem requires an _id property for deduplication")
            return
        }

        // Dédoublonnage: si l'item existe déjà, on le met à jour
        const existingItem = this.itemsMap.get(item._id)

        if (existingItem) {
            // Mise à jour de l'item existant
            Object.assign(existingItem, item)
        } else {
            // Nouvel item: on le pré-traite
            const processedItem = this.processItem(item)
            this.itemsMap.set(item._id, processedItem)
        }

        // Reconstruire le tableau items à partir de la Map
        this.state.items = Array.from(this.itemsMap.values())
            .sort((a, b) => (a._index || 0) - (b._index || 0))

        // Réappliquer les filtres pour mettre à jour filteredItems
        this.applyFilters()
    }

    /**
     * Supprime un item "fantôme" du store.
     * Utilisé lors du unmount d'un GhostItem.
     */
    unregisterGhostItem(id: string): void {
        if (!id) return
    ...
    /**
     * Pré-traite un item pour optimiser les recherches et filtres.
     */
    private processItem(item: GhostItemData): GhostItemData {
        if ((item as any)._processed) return item
    ...
    /**
     * @deprecated Utilisez registerGhostItem à la place pour le Ghost Pattern.
    ...
    setItems(items: any[]) {
        ...
        const processedItems = items.map((item, index) => {
            ...
            const id = item._id || item.id || `item_${index}`
        ...
        if (
            ...
            processedItems.every(
                (val, index) => val._id === this.state.items[index]?._id
            )
        ) {
            return
        }
        ...
        // Sync itemsMap
        this.itemsMap.clear()
        processedItems.forEach(item => this.itemsMap.set(item._id, item))
    ...
    private applyFilters() {
        ...
        let result = items.filter((item) => {
                ...
                if (filter.isMulti) {
                    if (
                        !this.checkMultiMatch(
                            filter.value,
                            (item as any)._multiIndex?.[filter.layerName]
                        )
                    )
                        return false
            ...
            // Apply search
            if (hasSearch) {
                const index = (item as any)._searchIndex || ""
...
if (typeof window !== "undefined" && !(window as any)[GLOBAL_STORE_KEY]) {
    (window as any)[GLOBAL_STORE_KEY] = new FilterStore()
}

export const store = (
    typeof window !== "undefined" ? (window as any)[GLOBAL_STORE_KEY] : new FilterStore()
) as FilterStore
